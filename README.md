...
Kahoot 式的**即時多人問答遊戲**：主持人建立遊戲、所有人用 6 位數代碼加入，
...
- 🎤 **主持人控台**：建立遊戲、選題數與每題秒數、控制出題節奏（適合投影到大螢幕）
- 🎮 **玩家畫面**：輸入遊戲代碼加入，手機／電腦皆可，所有裝置畫面即時同步
...
1. 主持人在首頁切到 **🎤 我要建立遊戲** → 設定題數與秒數 → 送出後畫面顯示 6 位數代碼。
...
- **建立遊戲時說「題庫是空的」** → 先到 `/#/admin` 後台新增題目，或重跑 `schema.sql`
  匯入範例題目。
