...
- 🎤 **主持人控台**：建立房間、選題數與每題秒數、控制出題節奏（適合投影到大螢幕）
- 🎮 **玩家畫面**：輸入房間代碼加入，手機／電腦皆可，所有裝置畫面即時同步
...
1. 主持人在首頁切到 **🎤 我要開房間** → 設定題數與秒數 → 建立房間，畫面顯示 6 位數代碼。
2. 玩家在首頁 **🎮 加入遊戲** → 輸入代碼 + 暱稱 + 密碼 → 進入等待室。
...
限制每題只計第一次。題庫管理則限定 `is_admin = true` 的帳號。
...
5. 到 **SQL Editor → New query**，貼上專案中 `supabase/schema.sql` 全部內容並 **Run**。
   這會建立資料表、權限規則、伺服器函式，把房間相關資料表加入 Realtime 推播，
   並塞入幾題範例題目。
...
- **建立房間時說「題庫是空的」** → 先到 `/#/admin` 後台新增題目，或重跑 `schema.sql`
  匯入範例題目。
...
- **遊戲開始後想加入** → 為了公平，開始後不能中途加入；已在房內的人重新整理後
  仍可回到現場（首頁會顯示「回到遊戲」）。
